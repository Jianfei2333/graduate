...
def update_q_value(G, action):
  ...
  q_table[current_state_index][action] = (1-alpha)*q + alpha*(reward+gamma*max_next_q)
  ...
  for k in range(0, 100):
    ...
    for i in range(0, 10):
      ...
      if q_table.__contains__(index):
        ...
        key = rand.randint(0, len(actions)-1)
        repair(Gt, key)
    maxlist.append(max_component(Gt))
    # print (max_component(Gt))
    # draw_graph(Gt)
  print (maxlist)
